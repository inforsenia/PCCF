...
5. Si hi ha canvi de PD, `notify_pd_teachers(cicle, familia, pdir)` avisa per correu el docent de cada mòdul amb incidències (mateix patró que `local_sync_poller.py::notify_teachers()` per a memòries, vore secció anterior). **v1: només mira el propi fitxer de PD** (placeholders `[###]`/`[...]` pendents + estat `_BORRADOR`), no la coherència de l'Excel. El docent identifica el seu email omplint la secció `### DOCENT` (`**Docent**:` / `**correu-e**:`) que porta la plantilla única de PD (`templates/_base_pd.md`) — mentre no ho òmpliga, `get_teacher_email()` (reutilitzat de `memories_utils.py`, mateixa regex `CORREU_E_RE`) torna `None` i no s'envia res. Estat persistent separat a `temp/pccf_pd_teacher_notify_state.json` (mtime per fitxer PD, no per cicle sencer), amb el mateix criteri de reintent que memòries: si `send_report_email()` falla, l'estat no es guarda.
   - **Plantilla única de PD** (des del curs 2026-27, decisió de departament: cap diferència entre cicles ni famílies): `templates/_base_pd.md`, usada per tots els cicles (`json2pccf.py`, `PD_TEMPLATE` a `pccf_utils.py`) i per les optatives (`json2optatives.py`, amb `optativa=True`). L'única part variable és la frase de context `ciclo_contexto`, que ve de `pccf_utils.py::CICLE_INFO` (els 7 cicles; un cicle nou s'hi ha d'afegir, si no json2pccf avisa i la frase queda buida) o `CONTEXTO_OPTATIVA`. Ja no hi ha plantilles per cicle (`PCCF_PD_Plantilla_MODULO_*.md`) ni bases per família (`_base_pd_{INF,SCO}.md`).
