...
make PCCF_ROOT=/path proyecto-dam  # pccf/ + programacions/ under an alternate root (OneDrive sync, see below)
...
All paths are relative to `$(PCCF_ROOT)` (default `.` = project root, or `pccf_sync` in OneDrive container).
...
- PD: `tools/report_pccf.py::is_pd_verified()` torna `False` si queda algun PD en BORRADOR, algun placeholder o Excel incoherent. S'usa a `compila-pd-pccf-%` via `tools/prepara_pd_compilacio.py` (escriu `$STAGE/.draft`, el Makefile passa `-V draft=true`).
- PD, a més: `prepara_pd_compilacio.py` marca el títol `# ` de cada mòdul (i per tant l'índex) amb ✎ (`\ding{46}`, BORRADOR) i ✗ vermell (`\ding{55}`, placeholders pendents o fulla Excel incoherent) -- equivalent a ✏️/❌ de memòries però amb `pifont`, perquè el PD no compila amb LuaTeX (el paquet `emoji` el requereix). També afig al final de cada mòdul el Quadre Resum de la seua fulla de `libro_{CICLE}.xlsx` (exportat amb `excel-to-pdfs.py::exportar_rango_a_pdf` + LibreOffice, `\includepdf[landscape=true]`). Tot sobre la còpia de muntatge `temp/compila_pd_{CICLE}/`, mai als fitxers dels docents. Des de la Fase 1 en dues fases (`json2pccf.py --generate-only`) el Quadre Resum ja no es generava enlloc; ara viu a la Fase 2.
...
5. Si hi ha canvi de PD, `notify_pd_teachers(cicle, familia, pdir)` avisa per correu el docent de cada mòdul amb incidències (mateix patró que `local_sync_poller.py::notify_teachers()` per a memòries, vore secció anterior). **v1: només mira el propi fitxer de PD** (placeholders `[###]`/`[...]` pendents + estat `_BORRADOR`), no la coherència de l'Excel. El docent identifica el seu email omplint la secció `### DOCENT` (`**Docent**:` / `**correu-e**:`) que ja porten les 8 plantilles actives (`templates_INF/PCCF_PD_Plantilla_MODULO_{SMX,DAM,CEIABD,FPBIIO}.md`, `templates_SCO/PCCF_PD_Plantilla_MODULO_{APD,EI,IS}.md`, `templates/PCCF_PD_Plantilla_MODULO_OPTATIVA.md`) — mentre no ho òmpliga, `get_teacher_email()` (reutilitzat de `memories_utils.py`, mateixa regex `CORREU_E_RE`) torna `None` i no s'envia res. Estat persistent separat a `temp/pccf_pd_teacher_notify_state.json` (mtime per fitxer PD, no per cicle sencer), amb el mateix criteri de reintent que memòries: si `send_report_email()` falla, l'estat no es guarda.
   - Les 4 plantilles INF (`SMX`/`DAM`/`CEIABD`/`FPBIIO`) i les 3 SCO (`APD`/`EI`/`IS`) ja no porten contingut propi: cada una és un `{% extends %}` cap a una base compartida per família (`templates_INF/_base_pd_INF.md`, `templates_SCO/_base_pd_SCO.md`) que conté tot el que abans estava duplicat literal. INF no té cap diferència real entre cicles (frase de context via `ciclo_contexto`, `CICLE_INFO_INF` a `pccf_utils.py`); SCO sí té contingut genuinament distint per cicle (metodologia, recursos, avaluació, etc.), exposat com a `{% block %}` que cada fitxer de cicle sobreescriu. `templates/PCCF_PD_Plantilla_MODULO_OPTATIVA.md` (català, genèric a tots els cicles) es manté fora d'esta jerarquia. `json2pccf.py::get_template_loader_and_env()` resol l'`extends` amb un `FileSystemLoader` de dos paths (família + genèric).
6. El cap de departament pot demanar la compilació del PDF de Programacions (`compila-pd-pccf-{cicle}`) sense accés al contenidor: crea un fitxer buit anomenat `COMPILAR_ARA` (també s'accepta `COMPILAR_ARA.md` / `COMPILAR_ARA.txt`, sense distingir majúscules: des d'OneDrive/Windows costa crear-lo sense extensió) dins de la seua carpeta sincronitzada `programacions/{CICLE}/`. `check_pd_compile_trigger()` el detecta a cada passada (independentment de si hi ha canvis), executa la compilació, i:
